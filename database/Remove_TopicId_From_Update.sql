-- Remove TopicId completely from Question_AdminUpdate - Always set to NULL
...
PRINT 'Creating updated Question_AdminUpdate procedure without TopicId validation...'
GO
...
CREATE PROCEDURE [dbo].[Question_AdminUpdate]
    ...
    @ExamId INT,
    @SubjectId INT,
    @TopicId INT, -- Parameter kept for compatibility but will be ignored
    @Marks DECIMAL(5,2),
...
BEGIN
    ...
    BEGIN TRY
        ...
        -- REMOVED: All topic validation logic - TopicId will always be set to NULL
        ...
        BEGIN
            ...
            -- Update main question - TopicId always set to NULL
            UPDATE Questions SET
                ModuleId = @ModuleId,
                ExamId = @ExamId,
                SubjectId = @SubjectId,
                TopicId = NULL, -- ALWAYS NULL - no topic validation needed
        ...
        ELSE
        BEGIN
            -- Update only basic fields without changing content - TopicId always set to NULL
            UPDATE Questions SET
                ModuleId = @ModuleId,
                ExamId = @ExamId,
                SubjectId = @SubjectId,
...
PRINT 'Question_AdminUpdate procedure updated - TopicId always set to NULL!'
GO
