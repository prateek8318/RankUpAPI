...
PRINT 'Fixing States Foreign Key - Corrected Version...';
PRINT '====================================================';
...
BEGIN
    ...
    SELECT TOP 5 CountryCode, COUNT(*) as StateCount 
    FROM dbo.States 
...
-- Check if States table has CountryId column instead
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM sys.columns WHERE object_id = OBJECT_ID('dbo.States') AND name = 'CountryCode')
    BEGIN
    ...
    -- Update CountryCode based on CountryId mapping
...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM sys.foreign_keys WHERE name = 'FK_States_Countries')
    BEGIN
...
-- Show final Countries data
PRINT 'Final Countries data:'
SELECT Id, Name, Iso2, CountryCode, PhoneLength, CurrencyCode, Image, IsActive 
FROM dbo.Countries 
ORDER BY Id
GO
