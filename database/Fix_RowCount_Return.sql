-- Fix the stored procedure to return correct row count
-- Remove SET NOCOUNT ON and add explicit row count return
...
PRINT 'Creating updated Question_AdminUpdate procedure with correct row count return...'
GO
...
AS
BEGIN
    -- REMOVED: SET NOCOUNT ON; - This was preventing row count from being returned
    BEGIN TRANSACTION;
    BEGIN TRY
        ...
        -- FIXED: Return explicit success indicator instead of @@ROWCOUNT
        SELECT 1 AS Success;
    END TRY
    BEGIN CATCH
...
PRINT 'Question_AdminUpdate procedure updated with correct success return!'
GO
