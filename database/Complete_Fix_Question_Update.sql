-- COMPLETE FIX FOR QUESTION UPDATE ISSUES
-- This script fixes all issues in one go - run this once only

USE [RankUp_QuestionDB]
GO

PRINT '=== STARTING COMPLETE FIX FOR QUESTION UPDATE ==='
GO

-- 1. Drop and recreate Question_AdminUpdate with all fixes
...
PRINT '1. Creating Question_AdminUpdate with all fixes...'
GO
...
AS
BEGIN
    -- NO SET NOCOUNT ON - We need to return success indicator
    BEGIN TRANSACTION;
    BEGIN TRY
        ...
        -- Return explicit success indicator
        SELECT 1 AS Success;
    END TRY
    BEGIN CATCH
...
PRINT '2. Question_AdminUpdate created successfully!'
GO

-- 3. Test the stored procedure
PRINT '3. Testing the stored procedure...'
GO

DECLARE @TestResult INT;
EXEC @TestResult = [dbo].[Question_AdminUpdate] 
    @Id=999999, -- Non-existent ID for safe test
    @ModuleId=1, @ExamId=21, @SubjectId=2, @TopicId=0,
    @Marks=1, @NegativeMarks=0, @Difficulty=2, @CorrectAnswer='A',
    @SameExplanationForAllLanguages=0, @IsPublished=0, @IsActive=1,
    @TranslationsJson='[]';

PRINT CONCAT('Test completed. Result: ', ISNULL(@TestResult, 'NULL'));
GO

PRINT '=== COMPLETE FIX APPLIED SUCCESSFULLY ==='
PRINT 'Now you only need to run this script once. The stored procedure will persist in the database.'
GO
